...
const ManageEntry = () => {
  ...
  const getEntries = async () => {
    try {
      const q = question.length > 0 ? `&question=${question}` : "";
      const u = user.length > 0 ? `&user=${user}` : "";

      const response = await axios.get(
        `${BASE_URL}/admin/entrys?page=${page}&limit=10${sortBy}${q}${u}`,
        {
  ...
  return (
    <View className="h-full">
      <ScrollView showsVerticalScrollIndicator={false} className="px-3">
        <Text className="mb-5 text-xl font-bold">All Entries</Text>
        <SelectDropdown
          data={sort}
        ...
        <FormField
          ...
          value={question}
          handleChangeText={(text: any) => setQuestion(text)}
          otherStyles="mb-3"
        />
        <FormField
          title="User"
          ...
          value={user}
          handleChangeText={(text: any) => setUser(text)}
          otherStyles="mb-3"
        />
        {entries.length === 0 ? (
